...
use Ethanzway\JWT\Exceptions\PayloadException;
...
class Payload implements ArrayAccess, Arrayable, Countable, Jsonable, JsonSerializable
{
     ...
     * @param  string  $claim
     *
     * @return \Ethanzway\JWT\Claims\Claim
     */
     ...
     * @param  mixed  $value
     *
     * @throws \Ethanzway\JWT\Exceptions\PayloadException
     */
    ...
    public function __call($method, $parameters)
    {
        if (preg_match('/get(.+)\b/i', $method, $matches)) {
            foreach ($this->claims as $claim) {
                if (get_class($claim) === 'Ethanzway\\JWT\\Claims\\'.$matches[1]) {
